...
#
class_name="New_Plugin_Framework"
short_name=$( echo ${class_name//_/-} | awk '{print tolower($0)}' )
server="eighty20results.com"
include=(class css js languages lib templates class-${class_name}.php README.txt)
exclude=(*.yml *.phar composer.* vendor)
build=(plugin-updates/vendor/*.php)
plugin_path="${short_name}"
version=$(egrep "^Version:" ../class-${class_name}.php | sed 's/[[:alpha:]|(|[:space:]|\:]//g' | awk -F- '{printf "%s", $1}')
